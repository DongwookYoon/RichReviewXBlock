  ...
  r2.course = (function() {
    ...
    var announcements = (function() {
      ...
      pub_an.init = function() {
        postCourse('getAnnouncements', { course_id: course_id })
    ...
    var surveys = (function() {
      ...
      pub_sv.init = function() {
        postCourse('getSurveys', { course_id: course_id })
    ...
    var submissions_instructor = (function() {
        ...
        pub_sbs.init = function() {
          return postCourse('getSubmissions', { course_id: course_id }).then(
            function(submissions_dict) {
              const submissions = []
              for (const id in submissions_dict) {
                if (submissions_dict.hasOwnProperty(id)) {
                  const sub = submissions_dict[id]
                  sub.id = id
      ...
      pub_si.enrollment = (function() {
        ...
        const removeDom = function(email) {
          $('#enrollment_items')
            .find('enrollment_' + email)
            .remove()
        }
        ...
        var addStudents = function(emails) {
          return postCourse('addEnrollment', {
            course_id: course_id,
            emails: emails
          })
        }
        ...
        var removeStudent = function(email) {
          return postCourse('removeEnrollment', {
            course_id: course_id,
            email: email
          })
        }
    ...
    var submission_student = (function() {
      ...
      pub_ss.init = function() {
        return pub_ss.submissions.init().catch(function(err) {
          Helper.Util.HandleError(err)
        })
      }
      ...
      pub_ss.submissions = (function() {
        ...
        pub_sss.init = function() {
          $('#submission_items')
            .find('tr')
            .remove()
          return postCourse('student_getSubmissions', {
            course_id: course_id,
            email: user_email
          }).then(function(submissions_dict) {
            const submissions = []
            for (const id in submissions_dict) {
              if (submissions_dict.hasOwnProperty(id)) {
                const sub = submissions_dict[id]
              ...
              $btn.click(function() {
                window.open(
                  host +
                    'viewer?' +
                    'access_code=' +
                    submission.group.pdf_hash +
                    '&docid=' +
                    submission.group.doc_id.substring(4) +
                    '&groupid=' +
                    submission.group.group_id.substring(4)
                )
              })
            } else {
              $review.text('Pending')
        ...
        const uploadPdf = (function() {
          ...
          pub_sssu.getFileInput = function() {
            const $input = createNewDomElement('input', [
              'file',
              'pdf-file-input'
            ])
          ...
          pub_sssu.getUploadBtn = function(submission_id) {
            const $btn = createNewDomElement('a', [
              'btn',
              'btn-default',
              'btn-sm'
            ])
          ...
          pub_sssu.link = function($btn, $file_input, submission_id) {
            ...
            const onerror = function(xhr, desc, err) {
              console.log(desc)
              console.log(err)
            ...
            const onload = function(evt) {
              postCourse('student__doneUploadPdf', {
                course_id: course_id,
                email: user_email,
            ...
            $btn.click(function() {
              ...
              postCourse('student__getUploadSas', {
                course_id: course_id,
                email: user_email,
                filename: filename
              })
    ...
    var reviews = (function() {
      ...
      pub_rv.init = function() {
        postCourse('getSubmissions', { course_id: course_id })
          .then(function(submissions_dict) {
            $('#reviews')
              .find('.panel-heading')
              .find('.title')
              .text(submissions_dict[review].title)
            return null
          })
          .then(review_items.init)
          .catch(function(err) {
            Helper.Util.HandleError(err)
          })
      }
      ...
      var review_items = (function() {
        ...
        pub_rvr.init = function() {
          return postCourse('getReviewItems', {
            course_id: course_id,
            review: review
          }).then(function(review_items) {
        ...
        var add = function(item) {
          ...
          const ready = item.status === 'ReadyForReview'
          const $tr = $(document.createElement('tr'))
          {
              ...
              if (ready) {
                ...
                $btn.click(function() {
                  window.open(
                    host +
                      ...
                      item.group.pdf_hash +
                      '&docid=' +
                      item.group.doc_id.substring(4) +
                      '&groupid=' +
                      item.group.group_id.substring(4)
                  )
                })
              } else if (submitted) {
                const $p = $(document.createElement('p'))
                ...
                $btn.click(function() {
                  alert(
                    'The student ' +
                      item.email +
                      ' has made the submission successfully in time. ' +
