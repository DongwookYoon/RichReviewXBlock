...
exports.COURSE_GROUPS = () => {
  return Object.keys(COURSE_GROUP).map(k => {
    return COURSE_GROUP[k]
  })
}

exports.admin_list = ['116730002901619859123']
...
exports.google_oauth = JSON.parse(
  fs.readFileSync(
    path.join(__dirname, '..', 'ssl/google_open_id.json'),
    'utf-8'
  )
)
...
exports.ssl_key = fs.readFileSync(
  path.join(__dirname, '..', 'ssl/richreview_net.key')
)
exports.ssl_cert = fs.readFileSync(
  path.join(__dirname, '..', 'ssl/richreview_net.crt')
)
...
exports.ubc = {
  idp_config: JSON.parse(
    fs.readFileSync(
      path.join(__dirname, '..', 'ssl/ubc_idp_config.json'),
      'utf-8'
    )
  ),
