...
exports.util = (function() {
  ...
  pub.keyExists = function(key) {
    return RedisClient.EXISTS(key).then(b => {
      return b === 1
    })
  }

  pub.isMember = function(key, value) {
    return RedisClient.SISMEMBER(key, value).then(b => {
      return b === 1
    })
  }
  ...
  pub.makeAtomic = (thisArg, fn) => {
    return function() {
      ...
      return genLock.lock(genLockResource, genLockTTL).then(lock => {
        return fn.apply(thisArg, myArguments).then(res => {
...

  {
    ...
    pub.GraphSet = function(graph, val1, val2) {
      ...
      return graphLock.lock(graphLockResource, graphLockTTL).then(lock => {
  ...
  pub.GraphGet = function(graph, val) {
    return RedisClient.HGET(graph, val).then(members => {
      if (members) {
...

  {
    ...
    const remove_from_list = (graph, member, val) => {
      ...
      return RedisClient.HGET(graph, member).then(o => {
      ...
      return RedisClient.HGET(graph, val)
        .then(members => {
          ...
          const promises = members.map(member => {
            return remove_from_list(graph, member, val)
          })
