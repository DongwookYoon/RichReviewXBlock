...
const DOC_LAYOUT_UPLOAD_URL = SERVER_URL + 'upload?mode=UploadDocLayout'

const loadJsScript = function(url, type) {
...
const FileList = (function() {
  ...
  const pub = {}
  const file_list = []
  const $ul_filelist = $(document.getElementById('ul_filelist'))
  const $btn_submit = $(document.getElementById('btn_submit'))
  let myuuid = ''
  let file_upload_error = false
  ...
  $btn_submit.click(function() {
    try {
      ...
      GetUuid()
        .then(UploadFiles)
        .then(MergePdfs)
        .then(loadMultiColumnAnalyzer)
        .catch(function(err) {
          $btn_submit.toggleClass('btn-primary', false)
          $btn_submit.toggleClass('btn-danger', true)
          $btn_submit.text('FAILED')
          alert(
            'Sorry, we failed to receive/analyze that PDF file. Please try again, and if the error recurs, please contact the system manager dy252@cornell.edu. Thank you.'
          )
  ...
  function UpdateDom() {
    try {
      ...
      for (var i = 0, f; (f = file_list[i]); i++) {
        ...
        li.appendChild(
          $(document.createElement('div')).toggleClass(
            'div-clear-both',
            true
          )[0]
        )
  ...
  function loadMultiColumnAnalyzer() {
    ...
    ;(function(Pla) {
      const doc_url = SERVER_URL + 'mupla_pdfs/' + myuuid + '/'
      Pla.ctx = {
      ...
      return loadJsScript('/static_multicolumn/load.js', 'js')
        .then(
...
const checkPlatform = function() {
  ...
  const is_supported_browser =
    bowser.chrome || bowser.firefox || bowser.safari || bowser.msedge
  if (is_mobile) {
    $('.container')
      .find('#multicolumn')
      .remove()
    ...
    $p.text(
      'Sorry! The upload feature is not supported on mobile platform yet. Please try again on your laptop or desktop.'
    )
    $well.append($p)
    ...
    var $p = $(document.createElement('h5'))
    $p.text(
      'Sorry! RichReview only supports Chrome, Firefox, Safari, or MS Edge browsers. But you are using something else...'
    )
    $well.append($p)
  }
}
