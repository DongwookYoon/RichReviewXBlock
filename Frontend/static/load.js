...
;(function(r2) {
  ...
  r2.constructErrorMsg = function(is_lti) {
    let msg =
      'Interesting... The system caught an unexpected error. Please wait for a couple of minutes,'
    if (is_lti) {
    ...
    msg +=
      " If it's the same, please copy-and-paste this error message and report this to the manager (dy252@cornell.edu)."
    return msg
  }
  ...
  r2.runSerialPromises = (function() {
    const runSerialPromises = function(promiseFuncs, rtns) {
      if (typeof rtns === 'undefined') {
        rtns = []
      }
      const run = promiseFuncs.shift()
      if (run) {
        return run()
          .then(function(rtn) {
            rtns.push(rtn)
  ...
  r2.loadApp = function(webapp_url_maps) {
    ...
    const run = function() {
      const scripts = [
        ...
        [
          r2.CDN_URL + '/lib/font-awesome-4.6.3/css/font-awesome.min.css',
          'css'
        ],
      ...
      const promises = scripts.map(function(script) {
        return function() {
          const is_absolute_path =
            script[0].indexOf('http://') === 0 ||
            script[0].indexOf('https://') === 0
      ...
      r2.runSerialPromises(promises)
        .then(function(rtns) {
          for (let i = 0; i < rtns.length; ++i) {
            if (rtns[i].type === 'error') {
              throw new Error(
                'Cannot load a resource file: ' + rtns[i].target.src
              )
            }
          }
          return r2
            .makeLocalJs(r2.cdn_endpoint + '/lib/pdfjs/pdf.worker.js') // prevent CORS issue
