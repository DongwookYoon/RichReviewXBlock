;(function(window) {
  ...
  const DEF_WORKER_PATH = 'lib_ext/recorder/recorderWorker.js'

  const Recorder = function(media_stream_source, cfg) {
    let recording = false
    let recording_residual = false
    let recording_captured = 0
    let recording_first_stream = false
    let recording_t_cmd = 0
    let recording_t_buf = 0
    let recording_stop_resp_channel_buf = false
    let recording_stop_resp_chunk_buf = false
    let recording_stop_cb = null
    ...
    this.node = (
      ...
      1,
      1 // # of input channel and output channel
    )

    const worker = new Worker(this.config.worker_path || DEF_WORKER_PATH)
    worker.postMessage({
      command: 'init',
      config: this.config
    })
    ...
    this.node.onaudioprocess = function(event) {
      if (recording) {
        ...
        if (recording_first_stream) {
          ...
          const d =
            (r2.audioRecorder.RECORDER_SOURCE_SAMPLE_RATE *
              (recording_t_buf - recording_t_cmd)) /
            1000
    ...
    worker.onmessage = function(e) {
      switch (e.data.command) {
        ...
        case 'recordChannelBufferResp':
          if (e.data.is_residual) recording_stop_resp_channel_buf = true
          stopAsync()
          break
    ...
    {
      const power_buf_size = this.config.buffer_size / 16
      const power = {
        sample: [],
      ...
      this.node_power.onaudioprocess = function(event) {
        if (recording) {
          const channel_buffer = event.inputBuffer.getChannelData(0)
          const arr = new Float32Array(channel_buffer)
          arr.set(channel_buffer, 0)
          const v = rootMeanSquare(arr, 0, arr.length)
          power.sample.push(v)
        } else if (power.sample.length !== 0) {
          power.sample = []
        }
      ...
      r2.audioRecorder.RECORDER_POWER_SAMPLE_PER_SEC =
        power_buf_size / this.context.sampleRate
    }
    ...
    this.exportWAV = function(cb) {
      ...
      if (!callbacks.onExportWav)
        throw new Error('Recorder callback set failed: onExportWav')
      worker.postMessage({
        command: 'exportWAV'
      })
    }
    ...
    this.getDbs = function(cb) {
      ...
      if (!callbacks.onGetDbs)
        throw new Error('Recorder callback set failed: onGetDbs')
