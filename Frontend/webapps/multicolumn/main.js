...
;(function(Pla) {
  ...
  Pla.ctrl = (function() {
    ...
    var initCtrl = function() {
      ...
      const promise = Pla.model.initModel(
    ...
    var runPla = function() {
      return Pla.model.getPdfPageData(cur_page).then(function(page) {
        ...
        preprocessTextBoxes(page.mupla_data, {
          w: page.pdfjs_canv.width,
          h: page.pdfjs_canv.height
        })

        const rects = page.mupla_data.GetRects()

        const alley_range = Pla.rectUtil.getAlleyRange(rects)

        render_ctx.pla_ctx = Pla.xyCut.run(rects, alley_range)

        const max_wght_group = Pla.multiColumn.runHistogramAnalysis(
          render_ctx.pla_ctx.block_group,
          alley_range
        )

        const textboxes_by_regions = Pla.multiColumn.categorizeTextBoxesIntoRegions(
          render_ctx.pla_ctx.block_group,
          max_wght_group
        )

        const texttearing_pieces = Pla.multiColumn.sliceRegionsIntoTextTearingPieces(
        ...
        render_ctx.ycut_blocks = Pla.rectUtil.getRectBlocksFromProjectedCuts(
          rects
        )
    ...
    var setText = function(tt_pieces, mupla_data) {
      const overlappingArea = function(x, y) {
        let l, t, r, b
        l = Math.max(x[0], y[0])
        t = Math.max(x[1], y[1])
        r = Math.min(x[2], y[2])
      ...
      const setTextLine = function(bbox, text) {
        let best = null
        let best_area = 0.0
        for (let i = 0, li = tt_pieces.length; i < li; ++i) {
          const region = tt_pieces[i]
          for (let j = 0, lj = region.rects.length; j < lj; j++) {
            const area = overlappingArea(bbox, region.rects[j])
            if (area > best_area) {
              best = region.rects[j]
              best_area = area
            }
      ...
      for (let i = 0, li = mupla_data.tblocks.length; i < li; ++i) {
        const block = mupla_data.tblocks[i]
    ...
    var preprocessTextBoxes = function(mupla, pdfjs_canv_size) {
      ...
      if (typeof mupla.resize_done === 'undefined') {
        const ratio_x = pdfjs_canv_size.w / (mupla.bbox[2] - mupla.bbox[0])
        const ratio_y = pdfjs_canv_size.h / (mupla.bbox[3] - mupla.bbox[1])
      ...
      const rects = mupla.GetRects()
      bbox = Pla.rectUtil.getRectsBBox(rects)
      const range_y = [bbox[3] - Pla.Param.PAGENUM_DETECTION_RANGE_Y, bbox[3]]

      for (i = 0; i < mupla.tblocks.length; ++i) {
        lines = mupla.tblocks[i].lines
        for (j = 0; j < lines.length; ++j) {
          const rect = lines[j].bbox
          if (
            /^([0-9]|\s)+$/.test(lines[j].text) && // number only text
            ...
            const exp_rect = [cx - w, cy - h, cx + w, cy + h]
  ...
  Pla.model = (function() {
    ...
    pub.getNumPages = function() {
      return num_pages
    }

    pub.initModel = function(pdf_url, js_url) {
      return getMuPlaJs(js_url).then(function() {
      ...
      return new Promise(function(resolve, reject) {
        Pla.util
          ...
          .then(function(js) {
            ...
            for (let i = 0; i < mupla_data.length; ++i) {
              mupla_data[i].GetRects = function() {
                const rects = []
                for (let i = 0; i < this.tblocks.length; ++i) {
                  const lines = this.tblocks[i].lines
    ...
    function getPdf(pdf_url) {
      return Pla.util
        .getUrlData(pdf_url, 'arraybuffer', null)
      ...
      return new Promise(function(resolve, reject) {
        if (page_data[n]) {
          ...
          pdf
            .getPage(n + 1)
            .then(function(page) {
              const s = Pla.Param.PAGE_W / page.pageInfo.view[2] // set page width
              const viewport = page.getViewport(s)
              const canv = document.createElement('canvas')
              const canv_ctx = canv.getContext('2d')
              const canv_pixel_ratio = Pla.util.getOutputScale(canv_ctx)

              canv.width = Math.floor(viewport.width * canv_pixel_ratio.sx) | 0
              canv.height =
                Math.floor(viewport.height * canv_pixel_ratio.sy) | 0
  ...
  Pla.View = (function() {
    ...
    const dr_canvas = document.getElementById('maincanvas')
    const dr_ctx = dr_canvas.getContext('2d')

    pub.Render = function(render_ctx, pdfjs_canv) {
      const margin = 10

      const scale = pdfjs_canv.height / $maincanvas.height()
    ...
    var renderInner = function(render_ctx, pdfjs_canv) {
      ...
      dr_ctx.fillText(
        render_ctx.n_page + 1 + ' / ' + render_ctx.n_page_total,
        470,
        30
      )

      if (display.rects) {
        const rects = render_ctx.rects
      ...
      if (display.ycut_blocks) {
        const ycut_blocks = render_ctx.ycut_blocks
        ycut_blocks.forEach(function(ycut_block) {
          const block = ycut_block.bbox
          dr_ctx.strokeStyle = 'rgba(0, 0, 255, 0.5)'
          dr_ctx.lineWidth = 2
          dr_ctx.beginPath()
          dr_ctx.rect(
      ...
      const pla_ctx = render_ctx.pla_ctx
      if (display.xcuts) {
      ...
      if (display.textbox_by_regions) {
        ...
        for (let i_rgn = 0; i_rgn < 4; ++i_rgn) {
          ...
          for (i = 0; i < pla_ctx.doublecolumn_rects[i_rgn].length; ++i) {
            const rect = pla_ctx.doublecolumn_rects[i_rgn][i]
            dr_ctx.fillRect(
              rect[0],
              rect[1],
              rect[2] - rect[0],
      ...
      if (display.texttearing_pieces) {
        // final cutting boxes
        for (let idx_rgn = 0; idx_rgn < 4; ++idx_rgn) {
          for (i = 0; i < render_ctx.multicolumn[idx_rgn].rects.length; ++i) {
            const box = render_ctx.multicolumn[idx_rgn].rects[i]
            if (i % 2 === 0) {
              dr_ctx.strokeStyle = 'rgba(255, 0, 0, 0.5)'
            } else {
            ...
            dr_ctx.lineWidth = 1
            dr_ctx.beginPath()
            dr_ctx.rect(box[0], box[1], box[2] - box[0], box[3] - box[1])
            dr_ctx.stroke()
          }
        }
      }
