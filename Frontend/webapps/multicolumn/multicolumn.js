...
;(function(Pla) {
   ...
   * @returns {*[]}
   */
  Pla.Histogram.prototype.init = function(
    rects,
    window_size,
    use_left_side,
    window_size_Lpf
  ) {
    const idx_side = use_left_side ? 0 : 2
    this.data = [
      { p: Number.NEGATIVE_INFINITY, wght: 0.0 },
      { p: Number.POSITIVE_INFINITY, wght: 0.0 }
    ]

    this.subslice(
      rects.map(function(r) {
        return r[idx_side] - window_size * 0.5
      })
    )
    this.subslice(
      rects.map(function(r) {
    ...
    const histogram_from_rect_height = rects.map(function(r) {
      return {
        seg: [r[idx_side] - window_size * 0.5, r[idx_side] + window_size * 0.5],
        wght: r[3] - r[1]
      }
    })
    ...
    for (let i = 0; (h = histogram_from_rect_height[i]); ++i) {
      for (let j = 0; j < this.data.length - 1; ++j) {
        if (
          Pla.rectUtil.testOverlapSegmentsIgnoreEdge(
            [this.data[j].p, this.data[j + 1].p],
            h.seg
          )
        ) {
    ...
    if (window_size_Lpf)
      // it's undefined when not running lpf
  ...
  Pla.Histogram.prototype.lowPassFilter = function(window_size_Lpf) {
    const n_cuts =
      Math.ceil(
        (this.data[this.data.length - 2].p - this.data[1].p) / window_size_Lpf
      ) + 1
    const origin_left = this.data[1].p
    const cut_pts = $.map($(new Array(n_cuts)), function(val, i) {
      return origin_left + window_size_Lpf * i
    })
    this.subslice(cut_pts)

    const new_histogram = $.map($(new Array(n_cuts)), function(val, i) {
      return { p: origin_left + window_size_Lpf * i, wght: 0 }
    })

    let j = 1
    for (let i = 0; i < new_histogram.length - 1; ++i) {
      while (this.data[j].p < new_histogram[i + 1].p) {
        new_histogram[i].wght +=
          this.data[j].wght * (this.data[j + 1].p - this.data[j].p)
        ++j
      }
  ...
  Pla.Histogram.prototype.getThreshold = function(scale) {
    ...
    this.cut_threshold =
      (scale * sum) / (this.data[this.data.length - 2].p - this.data[1].p)
  }
  ...
  Pla.Histogram.prototype.runThresholding = function() {
    ...
    for (let i = 0; i < this.data.length - 1; ++i) {
      if (
        this.data[i].wght < this.cut_threshold &&
        this.data[i + 1].wght >= this.cut_threshold
      ) {
        // where new block bgns
        new_blk = {
          range: [this.data[i + 1].p, 0, 0],
          wght: this.data[i + 1].wght
        }
      } else if (
        this.data[i].wght >= this.cut_threshold &&
        this.data[i + 1].wght >= this.cut_threshold
  ...
  Pla.multiColumn = (function() {
    ...
    pub.runHistogramAnalysis = function(block_group, alley_range) {
      for (let idx_block = 0; idx_block < block_group.length; ++idx_block) {
        const group = block_group[idx_block]
        ...
        const cuts = Pla.rectUtil.projectOnXaxisAndGetCuts(group.rects)
        const widest_cut = {
          w: 0,
          idx: -1,
          mid: 0
        ...
        if (
          widest_cut.w > Pla.Param.MIN_ALLEY_W && // (1) alley is wide enough
          ...
          group.histogram_left.init(
            group.rects,
            Pla.Param.EDGE_SPLATTER_HIST_L_WINDOW,
            (use_left_side = true)
          )
          group.histogram_left.getThreshold(
            Pla.Param.EDGE_SPLATTER_HIST_L_CUTOFF_RATIO
          )
          group.histogram_left.runThresholding()

          group.histogram_rght.init(
            group.rects,
            Pla.Param.EDGE_SPLATTER_HIST_R_WINDOW,
            (use_left_side = false),
            (window_size_Lpf = Pla.Param.EDGE_SPLATTER_HIST_R_LPF_W)
          )
          group.histogram_rght.getThreshold(
            Pla.Param.EDGE_SPLATTER_HIST_R_CUTOFF_RATIO
          )
          group.histogram_rght.runThresholding()

          // get the highst peak of the histogram
          group.histogram_max_wght = Math.max(
            group.histogram_left.thresholded_block.reduce(function(prev, cur) {
              return Math.max(prev, cur.wght)
            }, 0),
            group.histogram_rght.thresholded_block.reduce(function(prev, cur) {
              return Math.max(prev, cur.wght)
            }, 0)
          )
      ...
      const max_wght_group = {
        idx: -1,
        alley: null
      }

      max_wght_group.idx = block_group.reduce(function(
        prev_idx,
        cur_block_group,
        cur_idx
      ) {
        if (prev_idx >= 0) {
          if (
            block_group[prev_idx].histogram_max_wght >
            cur_block_group.histogram_max_wght
          ) {
            return prev_idx
          } else {
            return cur_idx
          }
        } else if (cur_block_group.histogram_max_wght > 0) {
          return cur_idx
        } else {
          return -1
        }
    ...
    pub.categorizeTextBoxesIntoRegions = function(block_group, max_wght_group) {
      ...
      if (max_wght_group.idx > -1) {
        // now i === idx_group_max_wght;
        let nj
        for (
          idx_rect = 0, nj = block_group[max_wght_group.idx].rects.length;
          idx_rect < nj;
          ++idx_rect
        ) {
          if (
            block_group[max_wght_group.idx].rects[idx_rect][0] <
            max_wght_group.alley[1]
          ) {
        ...
        for (; i >= 0; --i) {
          for (
            idx_rect = 0;
            idx_rect < block_group[i].rects.length;
            ++idx_rect
          ) {
        ...
        // if it's single column, put everything in the head.
    ...
    pub.sliceRegionsIntoTextTearingPieces = function(
      textboxes_by_region,
      doc_bbox,
      alley
    ) {
      // formatting
      const cut_pts_y = [[], [], [], []]
      const range_x = [[0, 0], [0, 0], [0, 0], [0, 0]]

      for (let i_rgn = 0; i_rgn < 4; ++i_rgn) {
        for (
          let idx_rect = 0;
          idx_rect < textboxes_by_region[i_rgn].length;
          ++idx_rect
        ) {
      ...
      let alley_x
      if (
        textboxes_by_region[LEFT].length === 0 &&
        textboxes_by_region[RGHT].length === 0
      ) {
        // no double columns, only header
        range_y = [doc_bbox[3], doc_bbox[3]]
        alley_x = 0.5 * (doc_bbox[0] + doc_bbox[2])
      } else {
        const left_bbox = Pla.rectUtil.getRectsBBox(textboxes_by_region[LEFT])
        const rght_bbox = Pla.rectUtil.getRectsBBox(textboxes_by_region[RGHT])
        range_y = [
          Math.min(left_bbox[1], rght_bbox[1]),
          Math.max(left_bbox[3], rght_bbox[3])
        ] // the double column's range
      ...
      for (idx_rgn = 0; idx_rgn < 4; ++idx_rgn) {
        cut_pts_y[idx_rgn] = cut_pts_y[idx_rgn].sort(function(a, b) {
          if (a < b) {
            return -1
          } else if (a > b) {
            return 1
        ...
        Pla.rectUtil.mergeOverlappingPts(
          cut_pts_y[idx_rgn],
          Pla.Param.MULTICOLUMN_MERGE_Y
        )
      ...
      const doublecolumn_text_boxes = [{}, {}, {}, {}]
      for (idx_rgn = 0; idx_rgn < 4; ++idx_rgn) {
        const rgn_bbox = Pla.rectUtil.getRectsBBox(textboxes_by_region[idx_rgn])
  ...
  Pla.xyCut = (function() {
    ...
    pub.run = function(rects, alley_range) {
      ...
      const ycut_blocks = Pla.rectUtil.getRectBlocksFromProjectedCuts(rects)
      ...
      for (i = 0; i < ycut_blocks.length; ++i) {
        xcuts.push(
          Pla.rectUtil.projectOnXaxisAndGetCuts(
            (rects = ycut_blocks[i].rects),
            alley_range
          )
        )
      }
      ...
      const xcut_seqs = {} // xcut_seqs["2,0,3,0,4,0"] = [{seq:[[2,0],[3,0],[4,0]], xcut: [-infinity, 132.0]}]

      const bbox_height = Pla.rectUtil.getRectsBBoxHeight(rects)
        ...
        if (!xcut_seqs.hasOwnProperty(seq)) {
          ...
          if (seq.length === 1) {
            ...
            v.xcut = Pla.rectUtil.intersectSegment(
              xcuts[seq[seq.length - 1][0]][seq[seq.length - 1][1]],
              BC_CascadeXCut(seq.slice(0, seq.length - 1))
            )
          }
          xcut_seqs[seq] = v
        }
      ...
      const BC_Overlap_seg_list = function(seg, list) {
        const l = []
      ...
      const BC_LargestOverlap_seg_list = function(seq, list) {
        const l = []
        for (let i = 0; i < list.length; ++i) {
      ...
      var BC = function(i, pX) {
        ...
        if (pX === NOSHARE) {
          ...
          const cascade_xcut = BC_CascadeXCut(pX)
          const overlap_list = BC_Overlap_seg_list(cascade_xcut, Xi)
          if (cascade_xcut[0] > cascade_xcut[1] || overlap_list.length === 0) {
            score = 0
          } else {
            const largest_seg = BC_LargestOverlap_seg_list(cascade_xcut, Xi)
            if (largest_seg[1] < largest_seg[0] + Pla.Param.MIN_ALLEY_W) {
              ...
              for (j = 0; j < overlap_list.length; ++j) {
                score = Math.max(
                  score,
                  BC(i + 1, pX.concat([[i, overlap_list[j]]]))
                )
              }
      ...
      if (verbose) {
        ...
        for (const bKey in memoization) {
          if (memoization.hasOwnProperty(bKey)) {
            console.log('Block:', bKey)
            const pKeys = Object.keys(memoization[bKey])
            for (i = 0; i < pKeys.length; ++i) {
              if (memoization[bKey].hasOwnProperty(pKeys[i])) {
                console.log(
                  '    ',
                  memoization[bKey][pKeys[i]].toFixed(2),
                  '    ',
                  pKeys[i] === -1
                    ? 'NOSHARE'
                    : JSON.stringify(xcut_seqs[pKeys[i]].seq),
                  pKeys[i] === -1
                    ? ' '
                    : JSON.stringify(xcut_seqs[pKeys[i]].xcut)
                )
              }
            }
          }
      ...
      // optimal sequence
      const group_seqs = getOptimalGroupSeqs(memoization, xcut_seqs)
      if (verbose) {
    ...
    var getOptimalGroupSeqs = function(memoization, xcut_seqs) {
      ...
      function Contains_MatchingSeq(seq, prev_seq_l) {
        ...
        for (let i = 0; (prev_seq = prev_seq_l[i]); ++i) {
          if (
            prev_seq !== NOSHARE &&
            prev_seq.equals(seq.slice(0, seq.length - 1))
          )
            return true
        }
        return false
      ...
      for (i = 1; i < memoization.length; ++i) {
        const possible_seq = []
        const seq_keys = Object.keys(memoization[i])
        var seq_key
        for (j = 0; (seq_key = seq_keys[j]); ++j) {
          if (seq_key === NOSHARE) {
            ...
            if (seq.length === 1) {
              if (Contains_NOSHARE(opt_seq[i - 1])) possible_seq.push(seq)
            } else if (Contains_MatchingSeq(seq, opt_seq[i - 1]))
  ...
  Pla.rectUtil = (function() {
    ...
    pub.testOverlapRects = function(ra, rb) {
      return (
        pub.testOverlapSegments([ra[0], ra[2]], [rb[0], rb[2]]) &&
    ...
    pub.getRectsBBoxHeight = function(rects) {
      const bbox = Pla.rectUtil.getRectsBBox(rects)
    ...
    pub.getAlleyRange = function(rects) {
      const x_com = getRectsCenterOfMass(rects)[0]
      const alley_range_width =
        Pla.Param.PAGE_W - 2.0 * Pla.Param.NARROW_COLUMN_W
    ...
    var getRectsCenterOfMass = function(rects) {
      ...
      let rect
      for (let i = 0; (rect = rects[i]); ++i) {
        const w = (rect[3] - rect[1]) * (rect[2] - rect[0])
      ...
      if (wsum === 0) {
        ...
        return [accum[0] / wsum, accum[1] / wsum]
      }
    }
    ...
    pub.getCutsOfSegments = function(segments, x_range) {
      ...
      const merged_segments = mergeOverlappingSegments(segments)
      ...
      cuts.push([
        merged_segments[merged_segments.length - 1][1],
        Number.POSITIVE_INFINITY
      ])
    ...
    var mergeSegmentsToSingle = function(segments) {
      const rtn = [Number.POSITIVE_INFINITY, Number.NEGATIVE_INFINITY]
     ...
     * @returns {*}
     */
    const projectAndCutRects = function(rects, range, cut_y) {
      let i = 0
      let j = 2
      if (cut_y) {
        i = 1
    ...
    pub.getRectBlocksFromProjectedCuts = function(rects) {
      let i, j
      const blocks = []
      const ycuts = pub.projectOnYaxisAndGetCuts(rects)

      const visited_rect = new Array(rects.length)
      for (j = 0; j < rects.length; ++j) {
        visited_rect[j] = false
      }
      for (i = 0; i < ycuts.length - 1; ++i) {
        const cut_range = [ycuts[i][1], ycuts[i + 1][0]]
        const block = {}
        block.rects = []
