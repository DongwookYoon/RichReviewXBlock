...
;(function(r2) {
  ...
  r2.newspeakUI = function(_textbox, _annotid, _annotids, _mycomment) {
    ...
    const $tbox = $(_textbox)
    let token_buffer = []
    let anchor_span = null
    let $anchor_span = null
    const annotid = _annotid
    let cur_gesture_replay = null
    let first = false
    const mycomment = _mycomment
    ...
    pub.drawDynamic = function(canvas_ctx) {
      if (cur_gesture_replay) {
        r2App.cur_page.dynamicSpotlightNewspeak(
          canvas_ctx,
          cur_gesture_replay,
          first
        )
                ...
                pub.Play(
                  function() {
                    r2.radialMenu.bgnLoading(
                      'rm_' + r2.util.escapeDomId(annotid)
                    )
                  },
                  function() {
    ...
    function onKeyDown(e) {
      if (e.keyCode === r2.keyboard.CONST.KEY_ENTER) {
        if (r2.keyboard.shift_key_dn) {
          // shift+enter starts new recording
          if (r2App.mode === r2App.AppModeEnum.RECORDING) {
          } else {
    ...
    function digestTokenBuffer(buf) {
      ...
      if (anchor_span) {
        ...
        buf.forEach(function(datum) {
          const span = $('<span>' + datum.word + '</span>')
          Object.keys(datum).forEach(function(key) {
            span.attr(key, datum[key])
          })
          $tbox.append(span)
    ...
    var indicators = (function() {
      ...
      const $wait = $('<span>Please wait.</span>')
      $wait.addClass('newspeak_wait').addClass('blink_me')

      const $speak = $('<span>Now Speak!</span>')
      $speak.addClass('newspeak_speak').addClass('blink_me')

      const $transcribing = $(
        '<span>Transcribing your speech. Please wait.</span>'
      )
    ...
    pub.Play = function(cbLoadingBgn, cbLoadingEnd) {
      ...
      function getGestureReplay(sentence) {
        ...
        const l = []
        for (let i = 0; i < sentence.length; ++i) {
          const span = sentence[i].s
          if (span) {
            const span_annotid = span.getAttribute('annotid')
            if (span_annotid) {
              const annot = r2App.annots[span_annotid]
              if (annot) {
                l.push({
                  annot: annot,
      ...
      for (var i = sentences.length - 1; i >= 0; --i) {
        if (sentences[i].length === 0) sentences.splice(i, 1)
      }
      ...
      r2.log.Log_Simple(
        'NewSpeak_PlayInside_' + anchor_n + '_' + sentences.length
      )

      var serialPlay = function(i) {
        if (i < sentences.length) {
          runForEachSpan(sentences[i], function(s) {
            $(s).addClass('replaying')
          })
          first = true
