...
;(function(r2) {
  r2.rich_audio = (function() {
    ...
    pub.play = function(annot_id, time) {
      ...
      r2.audioPlayer.play(
        annot_id,
        r2App.annots[annot_id].GetAudioFileUrl(),
        time,
        function() {},
        function() {}
      )
  ...
  r2.removeAnnot = function(annotid, askuser, mute) {
    ...
    if (
      rtn.reduce(function(accum_or, item) {
        return accum_or || item
      }, false)
    ) {
      if (!mute)
        alert(
          'The comment cannot be deleted since there is a subsequent comment underneath it.'
        )
    } else if (
      askuser === false ||
      ...
      if (r2App.annots[annotid]) {
        /* when a voice comment */
        delete r2App.annots[annotid]
      } else {
        /* when a typewritten comment */
      }
  ...
  r2.zoom = (function() {
    ...
    const scale_presets = [0.5, 0.75, 1.0, 1.25, 1.5]
    ...
    pub.touch = (function() {
      ...
      pub_tc.mv = function(p0, p1) {
        ...
        const s0 = anchor.scale
        const x0 = anchor.screenPos
        const t0 = anchor.scrollPos

        const s1 = r2.viewCtrl.scale
        const x1 = p0.add(p1, true).multiply(0.5)
        const t1 = new Vec2(
      ...
      function limitScale(scale) {
        const mn = scale_presets[0]
        const mx = scale_presets[scale_presets.length - 1]
    ...
    function zoomStep(delta) {
      ...
      function getClosestPresetScaleIdx(scale) {
        let i_min = 0
        let d_min = Number.POSITIVE_INFINITY
        for (let i = 0; i < scale_presets.length; ++i) {
          const d = Math.abs(scale - scale_presets[i])
          if (d < d_min) {
            i_min = i
            d_min = d
          }
  ...
  r2.turnPageAndSetFocus = function(searchresult, annotid) {
    ...
    r2.viewCtrl.setToFocus(
      new Vec2(piece.pos.x + piece.GetContentSize().x / 2, piece.pos.y)
    )
  ...
  r2.resizeWindow = function() {
    ...
    if (r2.dom_model.getCurPage()) {
      const x = r2.dom.getPosAndWidthInPage(r2.dom_model.getCurPage().get(0))
    ...
    const app_container_size = r2.dom.calcAppContainerSize()

    const scale = r2.viewCtrl.scale

    r2.viewCtrl.resizeView(app_container_size, doc_yx_ratio, {
      left: 0.0,
      rght: 0.0
    })
    ...
    r2.dom.resizeDom(
      scale,
      app_container_size,
      r2.viewCtrl.page_size_scaled,
      r2.viewCtrl.page_margins,
      r2.viewCtrl.canv_px_size
    )
  ...
  r2.speechUi = (function() {
    ...
    pub.init = function() {
      const cookie_setting = r2.util.getCookie('r2_speech_ui')
      if (
        cookie_setting === 'waveform' ||
    ...
    function reset() {
      const uis = ['.waveform', '.newspeak', '.simplespeech']
