...
;(function(r2) {
  ...
  r2.input = (function() {
    ...
    let mode = InputMode.DESKTOP
    let cursor_in_menu = false
    ...
    pub.outMenu = function() {
      cursor_in_menu = false
    }
    pub.isInMenu = function() {
      return cursor_in_menu
    }
  ...
  r2.pen = (function() {
    ...
    let mode = PenMode.NORMAL
    let cur_dn = false
    let pos_dn = new Vec2(0, 0)
    let pos_writing = new Vec2(0, 0)
    let pos_splight = new Vec2(0, 0)
    let cur_piece_tearing = null
    ...
    var dn = function(event) {
      ...
      const new_pen_pt = r2.input.getPos(event)
      const pos_px = new Vec2(event.clientX, event.clientY)
      if (r2App.mode === r2App.AppModeEnum.RECORDING) {
        r2.inkCtrl.recordingInkDn(
          r2.viewCtrl.mapScrToDoc(new_pen_pt),
          r2App.cur_recording_annot
        )
        pos_writing = pos_px
        ...
        if (r2.input.isTap(pos_dn, new_pen_pt)) {
          if (mode === PenMode.MANIPULATION) {
            if (!r2App.pieceSelector.isNull()) {
              r2.recordingCtrl.set(r2App.pieceSelector.get(), {
                ui_type: r2.speechUi.mode,
                log_type: 'PenTap'
              })
            }
      ...
      if (r2App.mode === r2App.AppModeEnum.RECORDING) {
        ...
        if (mode === PenMode.TEARING) {
          if (cur_piece_tearing) {
            const new_height = new_pen_pt.y - cur_piece_tearing.pos.y
            if (new_height >= 0) {
  ...
  r2.mouse = (function() {
    ...
    pub.setEventHandlers = function() {
      r2.dom.onMouseEventHandlers(pub.handleDn, pub.handleMv, pub.handleUp)
    }

    pub.removeEventHandlers = function() {
      r2.dom.offMouseEventHandlers(pub.handleDn, pub.handleMv, pub.handleUp)
    }

    pub.handleDn = function(event) {
      const new_mouse_pt = r2.input.getPos(event)
        ...
        switch (event.which) {
          case r2.MouseBtn.LEFT: // left click
            ...
            if (
              ...
              r2.spotlightCtrl.recordingSpotlightDn(
                r2.viewCtrl.mapScrToDoc(new_mouse_pt),
                r2App.cur_recording_annot
              )
            }
            break
    ...
    pub.rightClickContextMenu = (function() {
      ...
      pub_rc.enable = function() {
        document
          .getElementById('r2_app_container')
          .removeEventListener('contextmenu', disableFunc, false)
      }

      pub_rc.disable = function() {
        document
          .getElementById('r2_app_container')
          .addEventListener('contextmenu', disableFunc, false)
      }
  ...
  var handleTimeIndexingUp = function(pt) {
    const l = r2App.cur_page.HitTest(pt)
    if (l.length === 0) {
      return
    }
    ...
    if (obj_front instanceof r2.PieceAudio) {
      ...
      l.forEach(function(item) {
        if (item instanceof r2.Spotlight.Cache) {
          spotlights.push(item)
        }
      })
      for (let i = 0; (spotlight = spotlights[i]); ++i) {
  ...
  r2.keyboard = (function() {
    ...
    let mode = r2.KeyboardModeEnum.NORMAL
    ...
    pub.handleDn = function(event) {
      if (
        ...
        if (
          event.which === CONST.KEY_ENTER ||
          event.which === CONST.KEY_SPACE
        ) {
          // enter or space
          // for Recording_Stop() when key up;
        } else if (r2App.cur_recording_pieceaudios.length === 1) {
    ...
    pub.handleUp = function(event) {
      ...
      switch (event.which) {
        ...
        case CONST.KEY_SHIFT: // left shift
          ...
          break
        case CONST.KEY_PLUS:
          if (mode === r2.KeyboardModeEnum.NORMAL) r2.zoom.in()
          break
        case CONST.KEY_MINUS:
          if (mode === r2.KeyboardModeEnum.NORMAL) r2.zoom.out()
          break
        default:
          break
      }
    ...
    var modifierKeyCtrl = (function() {
      ...
      pub_mk.dn = function(which) {
        if (
          which === r2.EnvironmentDetector.is_mac
            ? CONST.KEY_CMD
            : CONST.KEY_CTRL
        ) {
          is_dn = true
  ...
  r2.onScreenButtons = (function() {
    ...
    const $dom = $('<div id="onscrbtns_static"></div>')

    pub.Init = function() {
      return r2.HtmlTemplate.loadOnce('onscrbtns').then(function(resp) {
        $dom.html(resp)
      })
    }
    ...
    pub.onClick = function(type) {
      if (type === 'keyboard') {
        ...
        } else if (!r2App.pieceSelector.isNull()) {
          r2.recordingCtrl.set(r2App.pieceSelector.get(), {
            ui_type: r2.speechUi.mode,
            log_type: 'OnScrBtn'
          })
  ...
  r2.inkCtrl = (function() {
    ...
    let cur_recording_Ink = null
    let cur_recording_Ink_segment = null
    let cur_recording_Ink_segment_piece = null
    let cur_recording_Ink_pt = null
    let cur_recording_Ink_piece = null
    ...
    pub.recordingInkDn = function(pt, target_annot) {
      const piece = r2App.cur_page.GetPieceByHitTest(pt)
    ...
    pub.eraser = (function() {
      ...
      let pos = null
      let eraser_dn_time = null
      let reserved_operations = []
      const uploader = new r2.CmdTimedUploader()
      ...
      pub_er.mv = function(pt) {
        ...
        r2App.invalidate_dynamic_scene = true
        if (inks.length) {
          for (let i = 0, l = inks.length; i < l; ++i) {
  ...
  r2.spotlightCtrl = (function() {
    ...
    let cur_recording_spotlight = null
    let cur_recording_spotlight_segment = null
    let cur_recording_spotlight_segment_piece = null
    let cur_recording_spotlight_pt = null
    ...
    pub.recordingSpotlightDn = function(pt, target_annot) {
      ...
      if (piece) {
        ...
        spotlight.SetSpotlight(
          ...
          r2App.cur_time,
          r2App.cur_time -
            target_annot.GetBgnTime() -
            r2App.cur_recording_asyn_delta_t,
          r2App.cur_time -
            target_annot.GetBgnTime() -
            r2App.cur_recording_asyn_delta_t
        )
  ...
  var replacePieceAudioToPieceKeyboard = function() {
    const annotid = r2App.cur_recording_annot.GetId()
  ...
  var createPieceTeared = function() {
    const anchorpiece = r2App.pieceSelector.get()
    if (anchorpiece) {
      if (
        anchorpiece instanceof r2.PieceTeared &&
        anchorpiece.getUsername() === r2.userGroup.cur_user.name
      ) {
        return anchorpiece
      } else {
        const creation_time = new Date(r2App.cur_time)
  ...
  var createPieceKeyboard = function(isprivate) {
    if (r2App.disable_comment_production) {
      alert(
        'This page is only for the review. Features for creating comments are disabled.'
      )
      return
    }
    ...
    if (r2App.mode === r2App.AppModeEnum.REPLAYING) {
      ...
      r2.rich_audio.stop()
      anchorpiece = r2App.cur_page.SearchPieceAudioByAnnotId(
        this._annotid,
        r2.audioPlayer.getPlaybackTime()
      )
    ...
    if (anchorpiece) {
      const annotid = new Date(r2App.cur_time).toISOString()
