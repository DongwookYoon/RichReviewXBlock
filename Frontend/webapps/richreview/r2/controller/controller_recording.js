...
;(function(r2) {
  r2.recordingCtrl = (function() {
    ...
    let triggered = false
    let anchor_piece = null
    let options = {} // has ui_type WAVEFORM, SIMPLE_SPEECH, OR NEW_SPEAK
    ...
    pub.set = function(_anchor_piece, _options) {
      ...
      r2.log.Log_Simple(
        'Recording_Bgn_' +
          _options.log_type +
          '_' +
          JSON.stringify(_options.ui_type)
      )
    }
          ...
          bluemix_stt
            ...
            .catch(function(err) {
              if (r2.ctx.lti) {
                // edX/cornellX
                err.custom_msg =
                  'You edX authentication session has timed out. Please go back to the course webpage at the edX.org, and retry.'
              } else {
                err.custom_msg =
                  'Needs login. Your RichReview authentication session has timed out.'
              }
              throw err
            })
  ...
  r2.recordingBgn = (function() {
    ...
    var run = function(anchor_piece, funcCreatePiece, options) {
      ...
      r2.Annot.prototype.SetAnnot.apply(
      ...
      /* create piece */
      funcCreatePiece(anchor_piece, annotid, options)
        .then(function() {
          ...
          r2App.cur_recording_asyn_delta_t =
            r2App.cur_time - r2App.cur_recording_asyn_delta_t
          if (r2App.cur_recording_piece.bgnCommentingAsync)
            r2App.cur_recording_piece.bgnCommentingAsync()
          if (options.piece_to_insert) {
            r2.dom_model.cbRecordingBgn(
              options.piece_to_insert.GetAnnotId(),
              'fa-stop'
            )
          } else {
            r2.dom_model.cbRecordingBgn(
              r2App.cur_recording_piece.GetAnnotId(),
              'fa-stop'
            )
          }
    ...
    var createPieceTranscription = function(
      anchor_piece,
      recording_annot_id,
      options
    ) {
      return new Promise(function(resolve, reject) {
        ...
        let piece_simple_speech = null
        if (options.piece_to_insert) {
          piece_simple_speech = options.piece_to_insert
        } else {
          const time_simple_speech = r2App.cur_time + 128
          const piece_annot_id = new Date(time_simple_speech).toISOString()
          const piece_annot = new r2.Annot()
          const args_new_annot = [
            piece_annot_id,
            anchor_piece.GetId(),
            time_simple_speech,
          ...
          setPieceFunc.apply(piece_simple_speech, args_set_piece_func)
  ...
  r2.recordingStop = (function() {
    ...
    var run = function(to_upload, funcOn, timed_out) {
      ...
      /* end audio recording */
      r2.audioRecorder.EndRecording().then(function(result) {
        r2.PieceAudio.prototype.NormalizePieceAudio(
          r2App.cur_recording_pieceaudios,
          (refresh_all = true)
        )

        /* annot */
        r2App.cur_recording_annot.SetRecordingAudioFileUrl(
          result.url,
          result.blob,
          result.buffer
        )
        ...
        if (timed_out) {
          alert(
            'You can record voice up to ' +
              Math.floor(r2Const.TIMEOUT_VOICE_COMMENTING / 1000) +
              ' sec long'
          )
        }
  ...
  r2.recordingUpdate = function() {
    ...
    const dbs = l[l.length - 1]
    {
      r2App.cur_recording_annot.UpdateDbs(dbs)
      r2.util
        .lastOf(r2App.cur_recording_pieceaudios)
        .UpdateAudioDbsRecording(
          r2App.cur_time - r2App.cur_recording_annot.GetBgnTime()
        )

      const timePerPiece =
        r2Const.PIECEAUDIO_TIME_PER_WIDTH *
        r2.util.lastOf(r2App.cur_recording_pieceaudios).GetTtIndentedWidth()
      const npiece = Math.ceil(
        r2App.cur_recording_annot.GetDuration() / timePerPiece
      )
      if (r2App.cur_recording_pieceaudios.length < npiece) {
        const anchorpiece = r2App.cur_recording_anchor_piece

        const pieceaudio = new r2.PieceAudio()
        const annot = r2App.cur_recording_annot
        pieceaudio.SetPiece(
