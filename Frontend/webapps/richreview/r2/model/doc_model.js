...
;(function(r2) {
  ...
  r2.Obj.prototype.AddChildrenChronologically = function(objs) {
    const time = objs[0].GetCreationTime()
    let i = 0
  ...
  r2.Obj.prototype.SearchPiece = function(id) {
    let i, child
    for (i = 0; (child = this.child[i]); ++i) {
      const rtn = child.SearchPiece(id)
      if (rtn) {
        return rtn
      }
    }
  ...
  r2.Obj.prototype.SearchPieceAudioByAnnotId = function(annotid, time) {
    let i, child
    for (i = 0; (child = this.child[i]); ++i) {
      const rtn = child.SearchPieceAudioByAnnotId(annotid, time)
      if (rtn) {
        return rtn
      }
    }
  ...
  r2.Obj.prototype.SearchPieceByAnnotId = function(annotid) {
    let i, child
    for (i = 0; (child = this.child[i]); ++i) {
      const rtn = child.SearchPieceByAnnotId(annotid)
      if (rtn) {
        return rtn
      }
    }
  ...
  r2.Obj.prototype.HitTest = function(pt) {
    ...
    if (this._isvisible) {
      ...
      if (
        this.pos.x < pt.x &&
        this.pos.x + this.size.x > pt.x &&
        this.pos.y < pt.y &&
        this.pos.y + this.size.y > pt.y &&
        this._isvisible
      ) {
        rtn.push(this)
      }
  ...
  r2.Doc.prototype.SearchPieceByAnnotId = function(annotid) {
    let i, page
    for (i = 0; (page = this._pages[i]); ++i) {
      const rtn = page.SearchPieceByAnnotId(annotid)
      if (rtn) {
        return { page_n: i, piece: rtn }
      }
    }
    return null
  }
  ...
  r2.Page.prototype.Relayout = function() {
    const rt = this.child[0]
    const rl = this.child[1]
    const rr = this.child[2]
    const rb = this.child[3]

    const s_rt = rt.Relayout(new Vec2(0, 0))
    const s_rl = rl.Relayout(new Vec2(0, s_rt.y))
    const s_rr = rr.Relayout(new Vec2(s_rl.x, s_rt.y))
    let mx = Math.max(s_rt.x, s_rl.x + s_rr.x)
    let my = s_rt.y + Math.max(s_rl.y, s_rr.y)
    const s_rb = rb.Relayout(new Vec2(0, my))
  ...
  r2.Page.prototype.refreshSpotlightPrerender = function() {
    this._spotlight_cache = []
    let i, spotlight, cache
    for (const annotid in r2App.annots) {
      if (
        (r2App.annots.hasOwnProperty(annotid) &&
          !r2App.annots[annotid].getIsBaseAnnot()) ||
        (r2App.cur_recording_annot !== null &&
          annotid === r2App.cur_recording_annot.GetId())
      ) {
        const annot = r2App.annots[annotid]
        const spotlights_of_page = annot.GetSpotlightsByNumPage(this._num)

        for (i = 0; (spotlight = spotlights_of_page[i]); ++i) {
          const segments = spotlight.getValidSegments()
          if (segments.length > 0) {
            const n_total_pts = segments.reduce(function(sum, item) {
              return sum + item.GetNumPts()
            }, 0)
            const t_step = (spotlight.t_end - spotlight.t_bgn) / n_total_pts

            let segment = segments[0]
            let cache_tbgn = spotlight.t_bgn
            let cache_pid = segment.GetPieceId()
            let cache_pts = segment.CopyPtsWithOffset(
              r2App.pieces_cache[segment.GetPieceId()].pos
            )

            for (let j = 1; j < segments.length; ++j) {
              segment = segments[j]
              if (
                !r2.Piece.prototype.isTheSameOrAdjecent(
                  segment.GetPieceId(),
                  cache_pid
                )
              ) {
                cache = new r2.Spotlight.Cache()
                const t_end_segment = cache_tbgn + cache_pts.length * t_step
                cache.setCache(annot, cache_tbgn, t_end_segment, cache_pts)
                this._spotlight_cache.push(cache)
                cache_pts = []
                cache_tbgn = t_end_segment
              }
              cache_pid = segment.GetPieceId()
              cache_pts = cache_pts.concat(
                segment.CopyPtsWithOffset(
                  r2App.pieces_cache[segment.GetPieceId()].pos
                )
              )
            }
            cache = new r2.Spotlight.Cache()
            cache.setCache(annot, cache_tbgn, spotlight.t_end, cache_pts)
            this._spotlight_cache.push(cache)
          }
        }
      }
    ...
    r2.spotlightRenderer.setCanvCtx(
      r2.viewCtrl.page_width_noscale,
      this.size.y / this.size.x
    )
    for (i = 0; (spotlight = this._spotlight_cache[i]); ++i) {
      spotlight.preRender(
        r2.spotlightRenderer.getCanvCtx(),
        r2.spotlightRenderer.getCanvWidth()
      ) // ctx, ratio
    }
  }
  ...
  r2.Page.prototype.dynamicSpotlightNewspeak = function(
    canvas_ctx,
    word_gestures,
    first
  ) {
    let spotlight
    for (let i = 0; (spotlight = this._spotlight_cache_newspeak[i]); ++i) {
      const valid_word_gestures = []
      for (var j = 0; j < word_gestures.length; ++j) {
        const word_gesture = word_gestures[j]
        if (word_gesture.annot === spotlight._annot) {
          if (
            !(
              word_gesture.bgn > spotlight._t_end ||
              word_gesture.end < spotlight._t_bgn
            )
          ) {
      ...
      let min_vwg = Number.MAX_VALUE
      let max_vwg = Number.MIN_VALUE
  ...
  r2.Page.prototype.refreshInkPrerender = function() {
    ...
    for (const annotid in r2App.annots) {
      if (r2App.annots.hasOwnProperty(annotid)) {
        const annot = r2App.annots[annotid]
        const Inks_of_page = annot.GetInksByNumPage(this._num)

        for (i = 0; (Ink = Inks_of_page[i]); ++i) {
          const segments = Ink.getValidSegments()
          if (segments.length > 0) {
            const n_total_pts = segments.reduce(function(sum, item) {
              return sum + item.GetNumPts()
            }, 0)

            const t_step = (Ink.t_end - Ink.t_bgn) / n_total_pts

            let segment = segments[0]
            let cache_tbgn = Ink._t_bgn
            let cache_pid = segment.GetPieceId()
            let cache_pts = segment.CopyPtsWithOffset(
              r2App.pieces_cache[segment.GetPieceId()].pos
            )
            ...
            cache = new r2.Ink.Cache()
            cache.setCache(annot, cache_tbgn, Ink._t_end, cache_pts)
            this._Ink_cache.push(cache)
          }
        }
      }
    }
    r2.InkRenderer.setCanvCtx(
      r2.viewCtrl.page_width_noscale,
      this.size.y / this.size.x
    )
  ...
  r2.Page.prototype.drawSpotlightPrerendered = function() {
    r2.canv_ctx.drawImage(
      ...
      0,
      1.0,
      r2.spotlightRenderer.getRenderHeight(
        this.size.y,
        r2.viewCtrl.page_width_noscale
      )
    )
  ...
  r2.Page.prototype.HitTest = function(pt) {
    const rtn = r2.Obj.prototype.HitTest.apply(this, [pt])
  ...
  r2.Region.prototype.Relayout = function(origin) {
    ...
    let my = 0
    let piece
    for (let i = 0; (piece = this.child[i]); ++i) {
      const s = piece.Relayout()
      mx = Math.max(mx, s.x)
      my += s.y
    }
  ...
  r2.Piece.prototype.Relayout = function() {
    const dom_piece =
      this._dom_piece === null
        ? $(document.getElementById(this.GetId())).get(0)
        : this._dom_piece

    const rect = r2.dom.getPosAndWidthInPage(dom_piece)
  ...
  r2.Piece.prototype.GetPieceOfClosestBottom = function(pt, dy_obj) {
    ...
    const dy = Math.abs(this.pos.y + this._cnt_size.y - pt.y)
    if (
      dy <= dy_obj[0] &&
  ...
  r2.Piece.prototype.SearchPiece = function(id) {
    ...
    const rtn = r2.Obj.prototype.SearchPiece.apply(this, [id])
    if (rtn) {
      return rtn
    }
    return null
  ...
  r2.Piece.prototype.getCollidingInks = function(pt, rtn) {
    let ink
    const pt_on_piece = pt.subtract(this.pos, true)
    let i, l
    for (const key in this._inks) {
  ...
  r2.Piece.prototype.detachInk = function(ink) {
    const inks = this._inks[ink._annotid]
    if (inks) {
      const idx = inks.indexOf(ink)
      if (idx > -1) {
        inks.splice(idx, 1)
      }
    } else {
  ...
  r2.Piece.prototype.drawInkReplaying = function(canvas_ctx) {
    ...
    for (const key in this._inks) {
      if (this._inks.hasOwnProperty(key)) {
        for (let i = 0; (ink = this._inks[key][i]); ++i) {
  ...
  r2.Piece.prototype.GetSelectedColors = function() {
    ...
    if (this._username && !this._isprivate) {
      const user = r2.userGroup.GetUser(this._username)
  ...
  r2.Piece.prototype.DrawSelected = function(canvas_ctx, x_offset) {
    ...
    if (this._visible) {
      ...
      const x_bgn = this.pos.x + this.GetTtIndent()
      let x0, x1, y
  ...
  r2.Piece.prototype.isTheSameOrAdjecent = function(pid0, pid1) {
    if (pid0 === pid1) {
      ...
      const piece0 = r2App.pieces_cache[pid0]
      const piece1 = r2App.pieces_cache[pid1]
  ...
  r2.PieceText.prototype.Relayout = function() {
    const rtn = r2.Piece.prototype.Relayout.apply(this, [])
    const pdf_x = Math.floor(this.pos.x * this._t_pdf_w)
    const pdf_y = Math.floor(this.pos.y * this._t_pdf_w)
  ...
  r2.PieceText.prototype.DrawPiece = function() {
    const canv = r2.pdfRenderer.getCanvas(this.GetNumPage())
    if (canv) {
      r2.canv_ctx.drawImage(
        canv,
        this._t_src_x,
  ...
  r2.PieceTeared.prototype.DrawPiece = function() {
    ...
    r2.canv_ctx.setLineDash([
      r2Const.PIECE_LINE_DASH * 0.75,
      r2Const.PIECE_LINE_DASH * 0.25
    ])
    r2.canv_ctx.beginPath()
    r2.canv_ctx.moveTo(x_bgn, this.pos.y)
    r2.canv_ctx.lineTo(x_bgn + this._ttW, this.pos.y)
    r2.canv_ctx.strokeStyle = r2.userGroup.GetUser(
      this._username
    ).color_normal_html
  ...
  r2.PieceAudio.prototype.SearchPieceByAnnotId = function(annotid) {
    const result = r2.Obj.prototype.SearchPieceByAnnotId.apply(this, [annotid])
    if (result) {
      return result
    } else if (this._annotid === annotid) {
      // ToDo check it returns the first piece
      return this
    } else {
      return null
    }
  ...
  r2.PieceAudio.prototype.SearchPieceAudioByAnnotId = function(annotid, time) {
    const rtn = r2.Obj.prototype.SearchPieceAudioByAnnotId.apply(this, [
      annotid,
      time
    ])
    if (rtn !== null) {
      return rtn
    } else if (this._annotid === annotid && time < this._t_end) {
      return this
    } else {
      return null
    }
  ...
  r2.PieceAudio.prototype.GatherPieceAudioByAnnotId = function(annotid) {
    let rtn = r2.Obj.prototype.GatherPieceAudioByAnnotId.apply(this, [annotid])
  ...
  r2.PieceAudio.prototype.SetPieceAudio = function(
    annotid,
    username,
    t_bgn,
    t_end
    ...
    const w = (this._t_end - this._t_bgn) / r2Const.PIECEAUDIO_TIME_PER_WIDTH
    const n = Math.floor(w / r2Const.PIECEAUDIO_STEP_W)

    this._audio_dbs = []
    for (let i = 0; i < n + 1; ++i) {
      const v =
        0.9 *
        this.GetAnnot().SampleAudioDbs(
          this._t_bgn + i * r2Const.PIECEAUDIO_STEP_T
        )
      if (v > 0) {
  ...
  r2.PieceAudio.prototype.UpdateAudioDbsRecording = function(t_end) {
    ...
    for (let i = this._audio_dbs_recording.length; i < n + 1; ++i) {
      const v = Math.max(
        0,
        0.9 *
          this.GetAnnot().SampleAudioDbs(
  ...
  r2.PieceAudio.prototype.DrawPiece = function() {
    const x_bgn = this.pos.x + this.GetTtIndent()
    const y_bgn = this.pos.y - r2Const.PIECEAUDIO_LINE_WIDTH
    ...
    if (this._isvisible) {
      ...
      for (let i = 0; i < this._audio_dbs.length; ++i) {
    ...
    let cnt_size_y = this._cnt_size.y
    if (!this._isvisible) {
      cnt_size_y = 0
    }
    r2.canv_ctx.beginPath()
    ...
    if (this._t_bgn === 0) {
      const pieceaudios = this.GetParent().GatherPieceAudioByAnnotId(
        this._annotid
      )
      const lastpiece = pieceaudios[pieceaudios.length - 1]
      const last_y =
        lastpiece.pos.y +
        lastpiece.GetContentSize().y -
        r2Const.PIECEAUDIO_LINE_WIDTH
      const ratio = Math.pow(0.8, this.GetTtDepth())
      const x_tip =
        this.pos.x +
        this.GetTtIndent() -
        (r2Const.RADIALMENU_OFFSET_X - r2Const.RADIALMENU_RADIUS) * ratio
      r2.canv_ctx.beginPath()
  ...
  r2.PieceAudio.prototype.DrawPieceDynamic = function(
    cur_annot_id,
    canvas_ctx,
    force
  ) {
    ...
    if (force) {
      ...
      n = Math.min(
        (r2App.cur_audio_time - this._t_bgn) / r2Const.PIECEAUDIO_STEP_T,
        this._audio_dbs.length
      )
    }
  ...
  r2.PieceAudio.prototype.GetPlayback = function(pt) {
    ...
    const dx = pt.x - this.pos.x - this.GetTtIndent()
    const t = this._t_bgn + r2Const.PIECEAUDIO_TIME_PER_WIDTH * dx
  ...
  r2.PieceAudio.prototype.FillUpAudioDbs = function() {
    for (
      let i = this._audio_dbs.length;
      i < this._audio_dbs_recording.length;
      ++i
    ) {
      r2App.cur_recording_minmax[0] = Math.min(
        this._audio_dbs_recording[i],
        r2App.cur_recording_minmax[0]
      )
      r2App.cur_recording_minmax[1] = Math.max(
        this._audio_dbs_recording[i],
        r2App.cur_recording_minmax[1]
      )
      const v =
        (0.9 * (this._audio_dbs_recording[i] - r2App.cur_recording_minmax[0])) /
        (r2App.cur_recording_minmax[1] - r2App.cur_recording_minmax[0])
      this._audio_dbs.push(v.toFixed(3))
    }
  }
  r2.PieceAudio.prototype.RefreshAudioDbs = function() {
    this._audio_dbs = []
    for (let i = 0; i < this._audio_dbs_recording.length; ++i) {
      r2App.cur_recording_minmax[0] = Math.min(
        this._audio_dbs_recording[i],
        r2App.cur_recording_minmax[0]
      )
  ...
  r2.PieceNewSpeak.prototype.ExportToNewBaseAnnot = function() {
    ...
    cmd.data.annot = r2App.annots[
      this.annotids[this.annotids.length - 1]
    ].ExportToCmd()
    return cmd
  }
  ...
  r2.PieceNewSpeak.prototype.CreateDom = function() {
    ...
    $(this.dom_tr).css(
      'left',
      this.GetTtIndent() * r2Const.FONT_SIZE_SCALE + 'em'
    )
    $(this.dom_tr).css(
      'width',
      this.GetTtIndentedWidth() * r2Const.FONT_SIZE_SCALE + 'em'
    )
    this.dom.appendChild(this.dom_tr)

    this.dom_textbox = document.createElement('div')
    this.dom_textbox.setAttribute(
      'contenteditable',
      this._username === r2.userGroup.cur_user.name
    )
    this.dom_textbox.classList.toggle('r2_piece_newspeak', true)
    this.dom_textbox.classList.toggle('text_selectable', true)
    this.dom_textbox.style.color = r2.userGroup.GetUser(
      this._username
    ).color_piecekeyboard_text
    this.dom_tr.appendChild(this.dom_textbox)

    this.dom_textbox.addEventListener(
      'focus',
      function(event) {
        r2App.cur_focused_piece_keyboard = this
        const color = r2.userGroup.GetUser(this._username)
          .color_piecekeyboard_box_shadow
        this.dom_textbox.style.boxShadow =
          '0 0 0.2em ' + color + ' inset, 0 0 0.2em ' + color
    ...
    this.newspeak = new r2.newspeakUI(
      this.dom_textbox,
      this._annotid,
      this.annotids,
      this._username === r2.userGroup.cur_user.name
    ...
    this.newspeak.insertRecording = function() {
      r2.recordingCtrl.set(this._parent, {
        // option
  ...
  r2.PieceNewSpeak.prototype.updateSizeWithTextInput = function() {
    ...
    const new_height = r2.viewCtrl.mapDomToDocScale(getHeight($(this.dom)))
  ...
  r2.PieceNewSpeak.prototype.DrawPiece = function() {
    ...
    r2.canv_ctx.lineTo(
      x_bgn + this.GetTtIndentedWidth(),
      y_bgn + this._cnt_size.y
    )
  ...
  r2.PieceNewSpeak.prototype.DrawPieceDynamic = function(
    cur_annot_id,
    canvas_ctx,
    force
  ) {
  ...
  r2.PieceNewSpeak.prototype.setCaptionFinal = function(words) {
    this.newspeak.setCaptionFinal(
      words,
      this.annotids[this.annotids.length - 1]
    ) // words and the base_annotid
    this.resizeDom()
  }
  ...
  r2.PieceNewSpeak.prototype.gatherSpotlights = function() {
    const annot = r2App.annots[this._annotid]
    annot._spotlights = []
    for (let i = 0; i < this.annotids.length; ++i) {
      const base_annot = r2App.annots[this.annotids[i]]
      for (let j = 0; j < base_annot._spotlights.length; ++j) {
  ...
  r2.PieceSimpleSpeech.prototype.DrawPieceDynamic = function(
    cur_annot_id,
    canvas_ctx,
    force
  ) {
  ...
  r2.PieceSimpleSpeech.prototype.bgnCommentingAsync = function(
    recording_annot_id
  ) {
  ...
  r2.PieceSimpleSpeech.prototype.setCaptionFinal = function(words) {
    this.simplespeech.setCaptionFinal(
      words,
      this.annotids[this.annotids.length - 1]
    )
    this.resizeDom()
  ...
  r2.PieceKeyboard.prototype.SetPieceKeyboard = function(
    anchor_pid,
    annotid,
    username,
    text,
  ...
  r2.PieceKeyboard.prototype.CreateDom = function() {
    ...
    /* add event handlers */
    const func_UpdateSizeWithTextInput = this.UpdateSizeWithTextInput.bind(this)
  ...
  r2.PieceKeyboard.prototype.Relayout = function() {
    this._isvisible =
      !this._isprivate || r2.userGroup.cur_user.name === this._username
    return r2.Piece.prototype.Relayout.apply(this, [])
  }
  r2.PieceKeyboard.prototype.DrawPiece = function() {
    let x_shift = this._isprivate ? this.GetPrivateShiftX() : 0
    if (!this._isvisible) {
      x_shift = -100
    }

    const x_bgn = this.pos.x + this.GetTtIndent() + x_shift
    const y_bgn = this.pos.y - r2Const.PIECEAUDIO_LINE_WIDTH

    if (this._isprivate && this._isvisible) {
      // private
      r2.canv_ctx.fillStyle = 'dimgray'
      if (x_shift < 0) {
        const x = x_bgn + this.GetTtIndentedWidth()
        r2.canv_ctx.fillRect(
          x,
          y_bgn,
          this.pos.x + this._cnt_size.x - x,
          this._cnt_size.y
        )
      } else {
        r2.canv_ctx.fillRect(
  ...
  r2.PieceKeyboard.prototype.UpdateSizeWithTextInput = function() {
    const realHeight = function($target) {
      const $next = $target.next()
      if ($next.length !== 0) {
        return $next.offset().top - $target.offset().top
      } else {
    ...
    const new_height = r2.viewCtrl.mapDomToDocScale(realHeight($(this.dom)))
    if (this._cnt_size.y !== new_height) {
      this._cnt_size.y = new_height
      return true
    }
  ...
  r2.Annot.prototype.ExportToCmd = function() {
    ...
    cmd.data.waveform_sample =
      this._ui_type === 'new_speak' ? [] : this._audio_dbs
    cmd.data.Spotlights = []
  ...
  r2.Annot.prototype.GetSpotlightsByNumPage = function(n) {
    return this._spotlights.filter(function(spotlight) {
      return spotlight.GetPage() === n
    })
  }
  r2.Annot.prototype.GetInksByNumPage = function(n) {
    return this._inks.filter(function(Ink) {
      return Ink.GetPage() === n
    })
  }
  ...
  r2.Annot.prototype.SetAnnot = function(
    id,
    anchorpid,
    t_bgn,
    duration,
  ...
  r2.Annot.prototype.detachInk = function(ink) {
    const idx = this._inks.indexOf(ink)
    if (idx > -1) {
  ...
  r2.Annot.prototype.SetRecordingAudioFileUrl = function(url, blob, buffer) {
    ...
    if (buffer) {
      ...
      const samples_per_sec = 256
      const n_chunk = Math.floor(
        r2.audioRecorder.RECORDER_SAMPLE_RATE / samples_per_sec + 0.5
      ) // 32 power samples per sec
      this._audio_dbs = []
      for (var i = 0, d = Math.floor(l.length / n_chunk); i < d; ++i) {
        this._audio_dbs.push(
          r2.util.rootMeanSquare(l, n_chunk * i, n_chunk * (i + 1))
        )
      }
      ...
      for (var i = 0; i < this._audio_dbs.length; ++i) {
        this._audio_dbs[i] = ((this._audio_dbs[i] - min) / (max - min)).toFixed(
          3
        )
      }
    }
  ...
  r2.Annot.prototype.SampleAudioDbs = function(msec) {
    const x = this._audio_dbs.length * (msec / this._duration)
    const p = x - Math.floor(x)
    const v0 =
      (1.0 - p) *
      this._audio_dbs[
        Math.max(0, Math.min(this._audio_dbs.length - 1, Math.floor(x)))
      ]
    const v1 =
      p *
      this._audio_dbs[
        Math.max(0, Math.min(this._audio_dbs.length - 1, Math.floor(x) + 1))
      ]
    return v0 + v1
  }
  r2.Annot.prototype.UpdateDbs = function(dbs) {
    ...
    const dbsPerSec = r2.audioRecorder.RECORDER_POWER_SAMPLE_PER_SEC
    const nDbs = Math.floor((r2App.cur_time - this._bgn_time) * dbsPerSec)

    for (let i = this._audio_dbs.length; i < nDbs; ++i) {
      this._audio_dbs.push(
        (r2.audioRecorder.RECORDER_SAMPLE_SCALE * dbs).toFixed(3)
      )
    }
  }
  ...
  r2.AnnotPrivateSpotlight.prototype.AddSpotlight = function(
    spotlight,
    toupload
  ) {
    if (spotlight.time in this._spotlightsDictionary) {
      return
    }
    this._spotlights.push(spotlight)
  ...
  r2.AnnotPrivateSpotlight.prototype.getCmdsToUpload = function() {
    if (
      this.changed &&
      r2App.cur_time - this.timeLastChanged >
        r2Const.TIMEOUT_PRIVATE_HIGHLIGHT_UPDATE
    ) {
      this.changed = false
  ...
  r2.AnnotStaticInk.prototype.addInk = function(ink, to_upload) {
    if (ink._t_bgn in this.inks_dict) {
      return
    }
  ...
  r2.AnnotStaticInk.prototype.getCmdsToUpload = function() {
    ...
    // type: 'StaticInk'
    // data: {inks: [Ink, Ink, ...] };
    const ink_cmds = this.add_ink_cmd_uploader.getCmdsToUpload()
    if (ink_cmds) {
      const cmd = {}
  ...
  r2.Ink.prototype.SetInk = function(
    pid,
    username,
    annotid,
    t_bgn_and_end,
  ...
  r2.Ink.prototype.erase = function(to_upload) {
    const piece = r2App.pieces_cache[this._pid]
    const annot =
      this._annotid === ''
        ? r2App.annots[
            r2.userGroup.GetUser(this._username).GetAnnotStaticInkId()
  ...
  r2.Ink.prototype.drawReplaying = function(canvas_ctx) {
    ...
    if (r2App.cur_annot_id !== null && r2App.cur_annot_id === this._annotid) {
      let n = Math.floor(
        (numall * (r2App.cur_audio_time - this._t_bgn)) /
          (this._t_end - this._t_bgn)
      )
      n = Math.min(numall, n)
      let curn = 0
      if (n >= 2) {
        ...
        canvas_ctx.strokeStyle = r2.userGroup.GetUser(
          this._username
        ).color_stroke_dynamic_past
  ...
  r2.Ink.Segment.prototype.CopyPtsWithOffset = function(offset) {
    const rtn = new Array(this._pts.length)
  ...
  r2.Ink.Segment.prototype.Draw = function(canvas_ctx, wasbgn) {
    ...
    if (piece) {
      ...
      for (let i = 0; i < this._pts.length; ++i) {
        if (wasbgn) {
          canvas_ctx.lineTo(
            offset.x + this._pts[i].x,
            offset.y + this._pts[i].y
          )
        } else {
          canvas_ctx.moveTo(
            offset.x + this._pts[i].x,
  ...
  r2.Ink.Segment.prototype.smoothing = function() {
    this._pts = r2.util.SimplifyStrokeDouglasPuecker(
      this._pts,
      0,
      this._pts.length,
      0.001
  ...
  r2.Ink.Cache.prototype.GetPlayback = function(pt) {
    if (
      ...
      for (let i = 0; i < this._pts.length - 1; ++i) {
        if (
          r2.util.linePointDistance(this._pts[i], this._pts[i + 1], pt) <
          r2Const.SPLGHT_WIDTH / 2
        ) {
          const rtn = {}
          rtn.annot = this._annot.GetId()
          rtn.t =
            this._t_bgn + (i / this._pts.length) * (this._t_end - this._t_bgn)
          return rtn
        }
      }
    }
  ...
  r2.Spotlight.prototype.SetSpotlight = function(
    username,
    annotid,
    npage,
    time,
  ...
  r2.Spotlight.Cache.prototype.setCache = function(annot, t_bgn, t_end, pts) {
    ...
    this._user = this._annot.GetUser()
    const max = new Vec2(Number.MIN_VALUE, Number.MIN_VALUE)
    const min = new Vec2(Number.MAX_VALUE, Number.MAX_VALUE)
    for (let i = 0; i < this._pts.length; ++i) {
      const v = this._pts[i]
    ...
    max.x += r2Const.SPLGHT_WIDTH / 2
    max.y += r2Const.SPLGHT_WIDTH / 2
    min.x -= r2Const.SPLGHT_WIDTH / 2
    min.y -= r2Const.SPLGHT_WIDTH / 2
    this._bb = [min, max]
  }
  ...
  r2.Spotlight.Cache.prototype.drawReplayBlob = function(canvas_ctx) {
    if (this._annot.GetId() === r2App.cur_annot_id) {
      if (
        ...
        let idx =
          ((this._pts.length - 2) * (r2App.cur_audio_time - this._t_bgn)) /
            (this._t_end - this._t_bgn) +
          0.25
        idx = Math.max(0, Math.min(this._pts.length - 2, Math.floor(idx)))

        const p0 = this._pts[idx]
        let p1
  ...
  r2.Spotlight.Cache.prototype.drawMovingBlob = function(
    p0,
    p1,
    forprivate,
    color,
