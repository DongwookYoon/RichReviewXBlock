...
;(function r2LagacyWrapper() {
  ...
  var DocObj = function(js) {
    this._pages = []
    for (let i = 0; i < js.pages.length; ++i) {
      const page = new Page(js.pages[i], i, null)
  ...
  DocObj.prototype.SearchPieceTearedWithAnnotId = function(annotId) {
    ...
    for (i = 0; (page = this._pages[i]); ++i) {
      const p = page.SearchPieceTearedWithAnnotId(annotId)
      if (p) {
  ...
  function Rect(js, parent) {
    if (js) {
      if (js.hasOwnProperty('iscmd')) {
        // from cmd
        const target_piece = pieces[js.target_piece]
        if (js.operation === 'create') {
          if (['PieceTeared', 'PieceKeyboard'].indexOf(js.datatype) >= 0) {
            this.trans = new Vec2(
              target_piece.trans.x,
              target_piece.trans.y + target_piece.size.y
            )
            this.scale = 1
  ...
  Rect.prototype.HitTest = function(pt) {
    ...
    if (
      this.t_absPtLt.x < pt.x &&
      this.t_absPtRb.x > pt.x &&
      this.t_absPtLt.y < pt.y &&
      this.t_absPtRb.y > pt.y
    ) {
      rtn.push(this)
    }
    return rtn
  ...
  Rect.prototype.SearchPieceProxyWithProxyId = function(id) {
    for (let i = 0; i < this._children.length; ++i) {
      const r = this._children[i].SearchPieceProxyWithProxyId(id)
      if (r) {
        return r
      }
    }
  ...
  Rect.prototype.SearchPieceTearedWithAnnotId = function(annotId) {
    for (let i = 0; (c = this._children[i]); ++i) {
      const r = c.SearchPieceTearedWithAnnotId(annotId)
      if (r) {
        return r
      }
    }
  ...
  Rect.prototype.GetPieceTearedOfAnnot = function(annotId) {
    for (let i = 0; i < this._children.length; ++i) {
      const r = this._children[i].GetPieceTearedOfAnnot(annotId)
      if (r !== null) {
        return r
      }
    }
  ...
  Page.prototype.LoadSpotlights = function(js) {
    ...
    for (let i = 0; i < js.length; ++i) {
      const l = []
      for (let j = 0; j < js[i].stroke.length; ++j) {
  ...
  Page.prototype.HitTest = function(pt) {
    ...
    for (const annot in this._sptlghts) {
      // for spotlights of all annotations
      if (this._sptlghts.hasOwnProperty(annot)) {
        for (i = 0; i < this._sptlghts[annot].length; ++i) {
          // ith spotlight of the annotation
          const splght = this._sptlghts[annot][i] // splght._ptsOnPieces[idx]
  ...
  Page.prototype.GatherSpotlightPosition = function() {
    ...
    for (const annot in this._sptlghts) {
      // for spotlights of all annotations
      if (this._sptlghts.hasOwnProperty(annot)) {
        for (let i = 0; i < this._sptlghts[annot].length; ++i) {
          // ith spotlight of the annotation
          const splght = this._sptlghts[annot][i] // splght._ptsOnPieces[idx]
          const p = splght.GetRepresentativePts()
          if (p) rtn.push(p)
        }
      }
    }
  ...
  Page.prototype.ExtractR2DocCmd_SpotlightByAnnotId = function(annotid) {
    if (this._sptlghts[annotid] === undefined) {
      return []
    }
    const spotlights = this._sptlghts[annotid]
    const rtn = []
    let i, spotlight
  ...
  function Region(js, parent) {
    ...
    for (let i = 0; i < js.pieces.length; ++i) {
      const typestr = js.pieces[i].Type
      ...
      if (js.hasOwnProperty('iscmd')) {
        ...
        if (js.operation === 'create') {
          if (['PieceTeared', 'PieceKeyboard'].indexOf(js.datatype) >= 0) {
            ...
            const tt_data = target_piece.GetTtDataSetForTtChild()
        ...
        this._stroke_pen = {}
        for (let i = 0; i < js.stroke_pen.length; ++i) {
          const js_strk = js.stroke_pen[i]
          const strokes = []
          for (let j = 0; j < js_strk.stroke.length; ++j) {
  ...
  Piece.prototype.ExportArgumentsToR2Doc_Piece = function() {
    return [
      ...
      0,
      this.size.clone(),
      [this._tt_level, this._tt_x0, this._tt_x1 - this._tt_x0]
    ]
  }
  Piece.prototype.GetNum_TtPieceGroup = function(_pid) {
    for (let i = 0; i < this._tt_childGroups.length; ++i) {
      const grp = this._tt_childGroups[i]
      if (grp.pidTeared === _pid) {
        // if this is a PieceTeared,
        return i
      }
      for (let j = 0; j < grp.pidNoneTeared.length; ++j) {
        // Otherwise
  ...
  Piece.prototype.GetSiblingTtTearedId = function() {
    const tt_n = pieces[this._tt_parent].GetNum_TtPieceGroup(this._id)
    const mygrp = this.GetSiblingTtPieceGroups()[tt_n]
    ...
    if (cmd.operation === 'create') {
      if (cmd.target_piece === this._id) {
        if (cmd.datatype === 'PieceTeared') {
          ...
          this._t_parent.AddPieceAfter(
            new_piece,
            new_piece.GetSiblingTtTearedId()
          ) // put this in the region
        }
      }
  ...
  Piece.prototype.SetTtPieceGroup = function() {
    ...
    if (this._t_tt_children_org.length !== 0) {
      const head = pieces[this._t_tt_children_org[0]]
      if (head.GetType() !== 'PieceTeared') {
        console.log('Warning from SetTtPieceGroup')
      }
      let grp = new TtPieceGroup(this._t_tt_children_org[0]) // pass pid teard

      for (let i = 1; i < this._t_tt_children_org.length; ++i) {
        const cur_piece = pieces[this._t_tt_children_org[i]]
        if (typeof cur_piece !== 'undefined') {
          // metro app bug
  ...
  Piece.prototype.HitTest = function(pt) {
    if (this._visible) {
      let rtn = Rect.prototype.HitTest.call(this, pt)
  ...
  Piece.prototype.HitTestStroke = function(pt) {
    let rtn = []
    for (const key in this._stroke_pen) {
      if (this._stroke_pen.hasOwnProperty(key)) {
        for (let i = 0; i < this._stroke_pen[key].length; ++i) {
      ...
      for (var i = 0; i < this._tt_childGroups.length; ++i) {
        if (this._tt_childGroups[i].pidTeared === _pid) {
          if (this._tt_childGroups[i].pidNoneTeared.length === 0) {
            ...
            console.log(
              'RemoveTtChild: cannot remove note when it has a comment underneath'
            )
          }
        }
      }
    ...
    if (this._visible) {
      for (const key in this._stroke_pen) {
        if (this._stroke_pen.hasOwnProperty(key) && key !== '') {
          for (let i = 0; i < this._stroke_pen[key].length; ++i) {
            const p = this._stroke_pen[key][i].GetRepresentativePts()
            if (p) {
              rtn.push(p)
            }
          }
  ...
  Piece.prototype.ExtractR2DocCmds_Ink = function() {
    ...
    for (const key in this._stroke_pen) {
      if (this._stroke_pen.hasOwnProperty(key)) {
        ...
        for (let i = 0; (stroke = this._stroke_pen[key][i]); ++i) {
          cmd_stroke = {}
  ...
  function PieceText(js, parent) {
    ...
    this._tex_coord_v0 = new Vec2(
      parseFloat(js.tex_coord_v0[0]),
      parseFloat(1.0 - js.tex_coord_v0[1])
    )
    this._tex_coord_v1 = new Vec2(
      parseFloat(js.tex_coord_v1[0]),
      parseFloat(1.0 - js.tex_coord_v1[1])
    )
  ...
  PieceText.prototype.DrSetPdf = function(pdf_canvas) {
    const pdf_x = Math.floor(this.t_absPtLt.x * pdf_canvas.width)
    const pdf_y = Math.floor(this.t_absPtLt.y * pdf_canvas.width)
    const pdf_w = Math.floor(
      (this.t_absPtRb.x - this.t_absPtLt.x) * pdf_canvas.width
    )
    const pdf_h = Math.floor(
      (this.t_absPtRb.y - this.t_absPtLt.y) * pdf_canvas.width + 1
    )
    ...
    const src_x = Math.floor(this._tex_coord_v0.x * pdf_canvas.width)
    const src_y = Math.floor((1.0 - this._tex_coord_v0.y) * pdf_canvas.height)
    this._t_src_w = pdf_w
    this._t_src_h = pdf_h
  ...
  PieceTeared.prototype.ExtractR2DocCmd_PieceTeared = function() {
    ...
    {
      cmd.time =
        this._annotId !== '' ? annots[this._annotId].GetRecordBgnTime() : 0
      cmd.user = r2Const.LEGACY_USERNAME
      cmd.op = 'CreateComment'
      cmd.type = 'TextTearing'
      cmd.data = {}
      const parent = pieces[this._tt_parent]
  ...
  PieceTeared.prototype.ExtractR2DocCmd_CommentAudio = function() {
    ...
    if (this._annotId !== '') {
      ...
      if (parent instanceof PieceText) {
        ...
        cmd.data.Spotlights = r2Legacy.doc.ExtractR2DocCmd_SpotlightByAnnotId(
          this._annotId
        )
        cmd.data.audiofileurl =
          r2App.file_storage_url + r2.ctx.pdfid + '/' + this._annotId + '.wav'
      } else if (parent instanceof PieceProxy) {
        cmd = {}
      } else {
        cmd = {}
  ...
  PieceTeared.prototype.SetVisibility = function(b) {
    ...
    const tt_n = pieces[this._tt_parent].GetNum_TtPieceGroup(this._id)
    const mygrp = this.GetSiblingTtPieceGroups()[tt_n]
    for (let i = 0; i < mygrp.pidNoneTeared.length; ++i) {
      pieces[mygrp.pidNoneTeared[i]].SetVisibility(b)
    }

    if (tt_n === 0) {
      const grps = this.GetSiblingTtPieceGroups()
      for (let j = 1; j < grps.length; ++j) {
  ...
  PieceProxy.prototype.GatherWaveformPosition = function() {
    ...
    if (typeof p !== 'undefined' && p.LongerThanHalf() && this._visible) {
      return [
        new Vec2(
          0.5 * (this._tt_x0 + this._tt_x1) + this.t_absPtLt.x,
  ...
  function Spotlight(js) {
    ...
    for (var i = 0; i < js.pts.length; ++i) {
      this._pts.push(
        new Vec2(parseFloat(js.pts[i][0]), parseFloat(js.pts[i][1]))
      )
    }
    this._ptsOnPieces = []
  ...
  Spotlight.prototype.ExtractR2DocCmd_Spotlight = function(npage) {
    ...
    for (let i = 1; i < this._ptsOnPieces.length; ++i) {
  ...
  Spotlight.prototype.GetPlayback = function(pt) {
    const t = this._time_bgn
    return [this._annotId, t]
  }
  Spotlight.prototype.GetRepresentativePts = function() {
    const pid = this._ptsOnPieces[0][1] // pid
    let v = this._ptsOnPieces[0][0] // vec
  ...
  function Annot(js) {
    this._id = js.id
    this._record_bgn_time = r2.util.epoch1601to1970(
      parseFloat(js.record_bgn_time[2]) / 10000.0
    )
    ...
    for (let i = 0; i < js.proxy_waveforms.length; ++i) {
      this._t_proxy_waveforms.push(
        new ProxyWaveform(this._js_proxy_waveforms[i], this._id, i)
      )
    }
  }
  ...
  Annot.prototype.GetAudioDbs = function(js) {
    const rtn = js.slice(0)
    const max = Math.max.apply(null, rtn)
    const min = Math.min.apply(null, rtn)
    if (max - min > 0.1) {
  ...
  Annot.prototype.SampleAudioDbs = function(msec) {
    const idx = Math.max(0, Math.floor((msec * 30.0) / 1000.0 + 0.5))
  ...
  function ProxyWaveform(js, _annot_id, i) {
    const annot_id = _annot_id
    let annot = null
    const t_bgn = parseFloat(js.time_bgn[2]) / 10000.0
    const t_end = parseFloat(js.time_end[2]) / 10000.0
    const height = js.height
    const id = Sha1.hash(annot_id + ' ProxyWaveform ' + i)
